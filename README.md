...
`task_complete` 生成的提醒 payload 会额外携带当前运行中任务数，固件会在显示提醒前同步更新主题共享的任务状态：
...
- `title` / `body`：提醒标题和正文。
- `run`：当前正在运行的 Codex 任务数量。固件将它写入共享 `DashboardViewModel`，由当前主题自行显示数字、状态灯或几何指示。
