...
- `hooks/codexmeter_stop_hook.py`：Codex `Stop` hook，用于任务完成提醒。
- `hooks/codexmeter_notify.py`：Codex `notify` 包装器，作为 `Stop` hook 未执行时的兜底提醒链路。
- `firmware/`：ESP32 固件，使用 PlatformIO、Arduino、LVGL、Arduino_GFX、ArduinoJson 和 NimBLE。
- `scripts/`：安装 hook 和 notify 包装器的辅助脚本。
- `install-mac.sh`：安装 macOS 端应用、LaunchAgent、Codex hook 和 notify 包装。
...
- 安装 Codex `Stop` hook 到 `~/.codex/hooks.json`，安装前会备份已有文件。
- 在 `~/.codex/config.toml` 中包装 `notify`，作为任务完成提醒兜底链路；如果原本已有 `notify` 命令，包装器会继续调用原命令。
...
- `~/.codex/config.toml` 中的 `notify` 是否已被包装。
- `~/.codex/hooks.json` 中是否存在 CodexMeter `Stop` hook。
