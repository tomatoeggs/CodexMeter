...
- `firmware/src/ui.*`：负责 LVGL 显示、余量卡片、电池图标、重置倒计时、正常页防烧屏漂移、红黄绿闪屏和任务完成视图。
...
1. UI 初始化时创建 `main_layer` 承载正常页的顶部信息、余量卡片和任务运行指示点。
2. 任务完成提醒层和亮度浮层仍挂在屏幕根节点上，不参与正常页漂移。
3. `ui_tick()` 每隔 10 分钟从固定偏移序列中选择下一个位置，并把 `main_layer` 移动到 2px 范围内的新坐标。
...
- 顶部左侧：Codex 标识。
- 顶部中间：双余量布局显示 `剩余用量`，Token 活动布局显示 `Token用量`。
- 顶部右侧：电量百分比和电池图标，填充色按电量变为绿色、黄色或红色。
- Codex 返回 5h 窗口时，第一张卡片显示 `5h 剩余:`、剩余百分比和 `HH:MM 后重置`；第二张卡片显示 `7d 剩余:`、剩余百分比和 `xd 后重置`。
- Codex 不返回 5h 窗口但返回有效 7d 数据时，第一张卡片显示 `今日用量` 和 `近7天` Token；第二张卡片显示 `7d 额度剩余`、剩余百分比和 `xd 后重置`。
