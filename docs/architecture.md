...
- `firmware/src/classic_theme.*` / `cyberpunk_theme.*` / `famicom_theme.*` / `animal_crossing_theme.*` / `gundam_theme.*` / `walle_theme.*` / `walle_v10_theme.*`：当前内置的七套独立 LVGL 仪表盘实现。
...
- `Classic`、`Cyberpunk`、`Famicom`、`Animal Crossing`、`Gundam`、`WALL-E` 与 `WALL-E EARTH` 主题使用独立 LVGL 对象树，但读取同一份 `DashboardViewModel`。
