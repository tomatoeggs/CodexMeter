...
6. 设置页依次包括主题、自动换主题、1–1440 分钟自动换主题间隔、亮度、预留音量和退出；30 秒无操作、任务提醒或关屏会退出，并恢复未确认编辑。
7. 自动换主题仅在亮屏、仪表盘可见且没有设置页、提醒、亮度或主题提示浮层时累计；暂停期间不丢失已累计的有效展示时间。
...
1. 仪表盘中左键 GPIO0 短按降低亮度，右键 GPIO18 短按增加亮度，长按左、右键分别切换并保存上一个、下一个主题；设置页编辑亮度时，两键短按调整预览值，长按不响应。
...
- 固件自动换主题间隔范围为 1–1440 分钟，默认 10 分钟且默认关闭；设置页无操作超时为 30 秒。
