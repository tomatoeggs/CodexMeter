...
- `Classic`、`Cyberpunk`、`Famicom`、`Animal Crossing`、`Gundam`、
  `GARGANTUA`、`WALL-E`、`WALL-E EARTH` 和 `WALL-E BLUEPRINT` 九套独立仪表盘。
...
| 场景 | 左 / 右短按 | 左 / 右长按 | 中间键短按 | 中间键长按 |
|---|---|---|---|---|
| Dashboard 且亮屏 | 快捷调整亮度 | 上一个 / 下一个主题 | 进入设置 | 关屏 |
| 设置页 Browse | 上 / 下选择设置项 | 忽略 | 进入编辑或执行开关 / 退出 | 关屏并取消未确认编辑 |
| 设置页 Edit | 减小 / 增大；主题项为前后预览 | 忽略 | 确认当前值 | 关屏并恢复编辑前值 |
| Completion / Startup | 忽略 | 忽略 | 忽略 | 关屏并结束当前临时场景 |
| 屏幕关闭 | 忽略 | 忽略 | 忽略 | 亮屏 |
...
2. 自动换主题：开关。
3. 自动换主题间隔：1 分钟–24 小时。
4. 屏幕亮度：10%–100%，10% 步进。
5. 音量：0%–100%。当前硬件路径尚未接入声音输出，该值先作为持久化能力预留。
6. 退出设置。
...
对于包含大量不可动插画细节、且逐个使用 LVGL 图元会明显损失还原度的主题，可以有条件使用“无字静态 RGB565 场景 + 透明动态 LVGL 覆盖层”。`Animal Crossing`、`GARGANTUA`、`WALL-E EARTH` 与 `WALL-E BLUEPRINT` 即采用这一方式：背景从 LittleFS 加载到 PSRAM，数值、标题、进度、电量和设备状态均是独立的透明实时对象；主题卸载时释放图片、字体和全部对象。Animal Crossing 针对当前 AMOLED 使用 70% 饱和度、94% 对比度的预校色背景；GARGANTUA 保留倾斜黑洞、吸积盘、引力透镜柔光与边框，所有业务文字均由 LVGL 覆盖；WALL-E EARTH 保留设计稿像素纹理，并以独立亮态、灰态和掩码资源更新十枚额度叶；WALL-E BLUEPRINT 保留工程线稿，并以独立亮态、暗态和格位掩码更新十格额度条与七格电池。未经校色的原始背景和带示例数据的设计参考均独立保留，后续适配其他面板时必须从原始参考重新生成，不能在运行时资源上重复处理。此类主题必须在注册前确认 LittleFS 容量、PSRAM 分配失败回退和反复切换后的内存稳定性。
...
- 实屏验证 Classic、设置页、Cyberpunk、Famicom、Animal Crossing、Gundam、GARGANTUA、WALL-E、WALL-E EARTH 与 WALL-E BLUEPRINT；差异化主题额外覆盖 100%、非 100%、6/7 个活动任务、电池与文本对齐。
...
- waiting / stale / 缺失字段。
- 电量低、中、高和充电。
- 0、1、2、6 个活动任务，以及主题可视指示器的上限与溢出状态。
