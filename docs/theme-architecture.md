...
本文描述 v3.0 已落地的 ESP32 多主题基础设施，以及后续主题化启动动画和任务完成页的扩展边界。主题系统只属于设备端展现层：macOS daemon 继续同步既有业务数据，BLE 协议不负责选择或保存主题。
...
- 任务完成页和启动动画保留主题接口；v3.0 使用系统默认版本。

## 2. 当前结构的约束

v3.0 已把原先的固定 UI 拆为主题无关的 ViewModel、运行时和 `classic` 主题，并以 `cyberpunk` 验证主题可以拥有完全不同的对象树、排版和动画。`ui.cpp` 只协调场景与系统浮层，不再通过主题分支维护两套控件。
...
v3.0 采用以下固件模块。后续规模扩大时可以在不改变接口的前提下移动到子目录：
...
- 实屏验证 Classic、设置页与 Cyberpunk；Cyberpunk 额外覆盖 100%、非 100%、6 个活动任务、电池与文本对齐。
